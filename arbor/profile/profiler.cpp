           ...
           std::string indent="")
{
    ...
    if (n.count==profile_node::npos) {
        snprintf(buf, util::size(buf), "_p_ %-20s%12s%12.7f%8.1f",
               name.c_str(), "-", float(n.time), proportion);
    }
    else {
        snprintf(buf, util::size(buf), "_p_ %-20s%12lu%12.7f%8.1f",
               name.c_str(), n.count, float(n.time), proportion);
    }
    o << "\n" << buf;
...
std::ostream& operator<<(std::ostream& o, const profile& prof) {
    ...
    snprintf(buf, util::size(buf), "_p_ %-20s%12s%12s%8s", "REGION", "CALLS", "WALL", "\%");
    o << buf;
